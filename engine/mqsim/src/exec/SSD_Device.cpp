...
SSD_Device::SSD_Device(Device_Parameter_Set *parameters, std::vector<IO_Flow_Parameter_Set *> *io_flows) : MQSimEngine::Sim_Object("SSDDevice")
{
	...
	case NVM::NVM_Type::FLASH:
	{
		...
		// BUG FIX (this project): Dynamic_Wearleveling_Enabled/Static_Wearleveling_
		// Enabled/Static_Wearleveling_Threshold were never passed here, so every
		// simulation silently used GC_and_WL_Unit_Page_Level's compiled-in
		// defaults (true, true, 100) no matter what ssdconfig.xml said - these
		// three config values were entirely unconfigurable. Only reachable in
